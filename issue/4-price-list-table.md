...
**要做：**
- 從 #7 的讀取端點 **`GET /api/pricing`** 取得價格資料載入前端（回傳形狀為 `docs/1-pricing-data-source.md` §4.2 的 `ModelPricing[]` ＋ `fetched_at`）。
...
- 單位顯示清楚（例如「USD / 1M tokens」）。**資料更新時間採端點回傳的 `fetched_at`**（我們抓取的時間）顯示。
- `cachedInputPricePerMTok` 或任何價格為 `null` 時，表格以「—」之類的占位顯示，**不要顯示成 0 或空白**（0 會被誤讀成免費）。
- 基本 RWD，手機上表格可正常閱讀。

**不做：**
- 不建資料層、不抓資料、不接 D1（那是 #7；本張只讀 `GET /api/pricing`）。
- 不做模型並排比較（那是 #5）。
- 不做成本試算器、不做歷史走勢圖。
...
- [ ] 頁面能從 `GET /api/pricing` 載入並列出模型，每列含 provider、模型名（displayName）、input 價、output 價、context window。
- [ ] 可依 input 價與 output 價排序，點擊欄位即時生效。
- [ ] 可依 provider 篩選（anthropic / openai / google），且關鍵字搜尋能即時過濾模型名稱。
- [ ] 價格單位明確標示，頁面顯示資料更新時間（採端點回傳的 `fetched_at`）。
...
- **依賴 #7**（價格資料層：D1 ＋ 排程 Worker ＋ `GET /api/pricing` 讀取端點）。本張只負責讀端點、畫表格，需 #7 完成且 D1 有資料後才開工。
